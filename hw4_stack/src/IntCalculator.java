...
public class IntCalculator {
    ...
    public String infixToPostfix() throws StackException {
        ...
        boolean previousIsDigit = false; // used to handle multi-digit numbers
        ...
        for (int i=0;i<infixExpression.length();i++) {
            ...
            if (isWhiteSpace(ch)){
                ...
                // ch is an operand
                if (!stringBuilder.isEmpty() && !previousIsDigit){
                    stringBuilder.append(' ');
                }
                previousIsDigit = true;
                stringBuilder.append(ch);
            } else if (ch == '(') {
                // ch is opening parenthesis
                previousIsDigit = false;
                myStack.push(ch);
            } else if (ch == ')') {
                // ch is closing parenthesis
                previousIsDigit = false;
                ...
                while (!myStack.isEmpty() && precedence(ch) <= precedence(myStack.peek())) {
                    char tmp = myStack.pop();
                    if (precedence(ch) == precedence(tmp) && precedence(ch)==3){
    ...
    public int evaluatePostfix()
        ...
        for (int i = 0; i < postfixExpression.length(); i++){
            ...
            if (isDigit(ch)){
                String value = "";
                while (i < postfixExpression.length() && postfixExpression.charAt(i)!=' '){
                    value += postfixExpression.charAt(i);
                    i++;
                }
                myStack.push(Integer.parseInt(value));
            } else if (isUnaryOperator(ch)) {
                int tmp = myStack.pop();
                myStack.push(-1*tmp);
            } else if (isBinaryOperator(ch)) {
                int tmp2 = myStack.pop();
                int tmp1 = myStack.pop();
                int res = 1;
                switch (ch) {
                    case '+':
                        res = tmp1 + tmp2;
